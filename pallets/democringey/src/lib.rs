...
pub trait ApprovingProposal {
	/// Able to approve a proposal represented by an id.
	fn approve_proposal(proposal_id: PropIndex) -> Vec<u8>;
}
...
pub trait Config: frame_system::Config + Sized {
	...
	/// The period between a treasury proposal being approved and enacted.
	///
	/// It doesn't need to be more than the unstake period since it should not affect individual 
	/// accounts in a way they would need to exit the system before the enactment if they are on
	///  the losing side of a vote.
	type TreasuryProposalsEnactmentPeriod: Get<Self::BlockNumber>;
	...
	/// A handle for creating a democracy proposal to approve a treasury proposal.
	type TreasuryProposals: ApprovingProposal;
}
...
decl_storage! {
	trait Store for Module<T: Config> as Democracy {
		...
		pub PublicProps get(fn public_props): Vec<(PropIndex, T::Hash, VoteThreshold, T::BlockNumber, T::AccountId)>;
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		/// accounts in a way they would need to exit the system before the enactment if they are on
		///  the losing side of a vote.
		const TreasuryProposalsEnactmentPeriod: T::BlockNumber = T::TreasuryProposalsEnactmentPeriod::get();
		...
		fn propose(origin,
			...
			<PublicProps<T>>::append((index, proposal_hash, VoteThreshold::SuperMajorityApprove, 
				T::EnactmentPeriod::get(), who));
		...
		fn propose_treasury_spend_simple_majority(origin,
			...
			PublicPropCount::put(index + 1);
			<DepositOf<T>>::insert(index, (&[&who][..], value));
			<PublicProps<T>>::append((index, proposal_hash, VoteThreshold::SimpleMajority, 
				T::TreasuryProposalsEnactmentPeriod::get(), who));
			Self::deposit_event(RawEvent::Proposed(index, value));
		}
...
impl<T: Config> Module<T> {
	...
	fn launch_public(now: T::BlockNumber) -> DispatchResult {
		...
		{
			let (prop_index, proposal, threshold, enactment_period, _) = public_props.swap_remove(winner_index);
